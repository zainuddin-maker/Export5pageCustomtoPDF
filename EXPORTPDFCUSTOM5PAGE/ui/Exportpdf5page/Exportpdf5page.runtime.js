TW.Runtime.Widgets.Exportpdf5page = function () {
    this.renderHtml = function () {
        // return any HTML you want rendered for your widget
        // If you want it to change depending on properties that the user
        // has set, you can use this.getProperty(propertyName). In
        // this example, we'll just return static HTML
        return '<div class="widget-content widget-Exportpdf5page"></div>';
    };

    // };

    this.updateProperty = function (updatePropertyInfo) {
        // TargetProperty tells you which of your bound properties changed

        this.setProperty(
            updatePropertyInfo.TargetProperty,
            updatePropertyInfo.SinglePropertyValue
        );


        if (updatePropertyInfo.TargetProperty === "ConfigurationWidth") {
            this.setProperty(
                "ConfigurationWidth",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
         
        }

        if (updatePropertyInfo.TargetProperty === "LabourProductivity") {
            this.setProperty(
                "LabourProductivity",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }

        if (updatePropertyInfo.TargetProperty === "DdsInformation") {
            this.setProperty(
                "DdsInformation",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "LoadingSchedule") {
            this.setProperty(
                "LoadingSchedule",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "FormerTemparature") {
            this.setProperty(
                "FormerTemparature",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "PlcReading") {
            this.setProperty(
                "PlcReading",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "ChemicalMixRmNumber") {
            this.setProperty(
                "ChemicalMixRmNumber",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "ConfigurationConcentrationFlowmeter") {
            this.setProperty(
                "ConfigurationConcentrationFlowmeter",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "ConfigurationConcentrationFlowmeter") {
            this.setProperty(
                "ConfigurationConcentrationFlowmeter",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "DataTableConcentration") {
            this.setProperty(
                "DataTableConcentration",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "DataTableFlowmeter") {
            this.setProperty(
                "DataTableFlowmeter",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "DataTableLatexInformation") {
            this.setProperty(
                "DataTableLatexInformation",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "ConfigurationLatexInformation") {
            this.setProperty(
                "ConfigurationLatexInformation",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "DataTableDippingTemperature") {
            this.setProperty(
                "DataTableDippingTemperature",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "ConfigurationDippingTemperature") {
            this.setProperty(
                "ConfigurationDippingTemperature",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "DataTableDowntime") {
            this.setProperty(
                "DataTableDowntime",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
        if (updatePropertyInfo.TargetProperty === "ConfigurationDowntime") {
            this.setProperty(
                "ConfigurationDowntime",
                updatePropertyInfo.RawDataFromInvoke.rows
            );
           
        }
      
      
      

        this.setupWidget();
    };

    this.afterRender = function () {
        this.setupWidget();
    };

    this.serviceInvoked = function (serviceName) {


        try {
            var allWidgetProps = this.properties;

            var widgetProps = {};

            for (const [key, value] of Object.entries(allWidgetProps)) {
                if (key.includes("Style")) {
                    widgetProps[key] = TW.getStyleFromStyleDefinition(
                        this.getProperty(key)
                    );
                } else {
                    widgetProps[key] = this.getProperty(key);
                }
            }

            console.log("widgetProps trigger running table");
            console.log(widgetProps);
        } catch (error) {
            console.log("error");
            console.log(error);
        }

       
      
      
        d3.select("#buttonimage" + (widgetProps.idRandom) ).on("click")();
        

      
    };

    this.setupWidget = function () {
        var widgetID = this.jqElementId;
        console.log("widgetID")
        console.log(widgetID)
        // Remove all old/existing DOM element
        d3.select(`#${widgetID}`).selectAll("*").remove();
        // Handle Properties
        try {
            var allWidgetProps = this.properties;

            var widgetProps = {};

            for (const [key, value] of Object.entries(allWidgetProps)) {
                if (key.includes("Style")) {
                    widgetProps[key] = TW.getStyleFromStyleDefinition(
                        this.getProperty(key)
                    );
                } else {
                    widgetProps[key] = this.getProperty(key);
                }
            }

            console.log("widgetProps running table");
            console.log(widgetProps);
        } catch (error) {
            console.log("error");
            console.log(error);
        }
      

        // var booleandisplaybutton = widgetProps.BooleanDisplayButton
        //     ? widgetProps.BooleanDisplayButton
        //     : false;

        // var configuration = widgetProps.Configuration
        //     ? widgetProps.Configuration
        //     : [];

        // var data_from_thingworx = widgetProps.DataThingworx
        //     ? widgetProps.DataThingworx
        //     : [];

        // var doc_information = [];

        // var jsondocinfomration =
        //     typeof widgetProps.JSONDocinformation == "string"
        //         ? JSON.parse(widgetProps.JSONDocinformation)
        //         : widgetProps.JSONDocinformation;

        // for (x in jsondocinfomration) {
        //     doc_information.push({ name: x, value: widgetProps[x] });
        // }

        // var widhtimage = widgetProps.Widhtimage;
        // var heightimage = widgetProps.Heightimage;
        // var booleanpdf = widgetProps.Booleanpdf;
        // var booleanexcell = widgetProps.Booleanexcell;

        // var file_name = widgetProps.Filename;

        // var file_name_id = widgetProps.Id;


        // var header_name = widgetProps.Headername;

        // var header_information = [];
        // var jsonheader_information =
        //     typeof widgetProps.JSONHeaderinformation == "string"
        //         ? JSON.parse(widgetProps.JSONHeaderinformation)
        //         : widgetProps.JSONHeaderinformation;

        // for (x in jsonheader_information) {
        //     header_information.push({ name: x, value: widgetProps[x] });
        // }

        // var dashboard_information = [];

        // var jsondashboard_information =
        //     typeof widgetProps.JSONDashboardinformation == "string"
        //         ? JSON.parse(widgetProps.JSONDashboardinformation)
        //         : widgetProps.JSONDashboardinformation;
        // //console.log("jsondashboard_information")
        // //console.log(jsondashboard_information)

        // for (x in jsondashboard_information) {
        //     //console.log("x")
        //     //console.log(x)
        //     dashboard_information.push({ name: x, value: widgetProps[x] });
        // }
        // //console.log("dashboard_information")
        // //console.log(dashboard_information)

        //batas inpput

        //allpage
var idRandom = widgetProps.idRandom ;



// var doc_information = [
//     {
//         name: "AIMS Document No",
//         value: "A-EX-G6-1-FF-G",
//     },
//     {
//         name: "Page",
//         value: "Page 1 of 1",
//     },
//     {
//         name: "Generated Date",
//         value: "18.03.2022",
//     },
// ];

 var doc_information = [];

        var jsondocinfomration =
            typeof widgetProps.JSONDocinformation == "string"
                ? JSON.parse(widgetProps.JSONDocinformation)
                : widgetProps.JSONDocinformation;

        for (x in jsondocinfomration) {
            doc_information.push({ name: x, value: widgetProps[x] });
        }

// var header_information = [
//     {
//         name: "Country",
//         value: "Malaysia",
//     },
//     {
//         name: "Line/Machine",
//         value: "MS3",
//     },
//     {
//         name: "Shift",
//         value: "AM",
//     },
//     {
//         name: "Glove Type",
//         value: "PI SKF White",
//     },
//     {
//         name: "Dipping Batch No",
//         value: "123456789",
//     },
//     {
//         name: "Dipping Supervisor",
//         value: "Malaysia",
//     },
//     {
//         name: "Shift Superintendent",
//         value: "MS3",
//     },
//     {
//         name: "Verified Date",
//         value: "18 03 2022",
//     },
//     {
//         name: "Stopwatch ID",
//         value: "05-AM-DT-50",
//     },
//     {
//         name: "Thermometer ID",
//         value: "05-AM-DT-50",
//     },
// ];


  var header_information = [];
        var jsonheader_information =
            typeof widgetProps.JSONHeaderinformation == "string"
                ? JSON.parse(widgetProps.JSONHeaderinformation)
                : widgetProps.JSONHeaderinformation;

        for (x in jsonheader_information) {
            header_information.push({ name: x, value: widgetProps[x] });
        }

var configuration_width = widgetProps.ConfigurationWidth ||  []

var dashboard_information = [];

    var booleandisplaybutton = widgetProps.BooleanDisplayButton
            ? widgetProps.BooleanDisplayButton
            : false;

var widhtimage = 20;
var heightimage = 20;
// var booleanpdf = true;
// var booleanexcell = true;

var file_name = widgetProps.Filename || "filename-data";
 var header_name = widgetProps.Headername || "";

//
//page 1

   //input infotable
   var labour_productivity = widgetProps.LabourProductivity || []
    
var dds_information =  widgetProps.DdsInformation || []
var loading_schedule =  widgetProps.LoadingSchedule  || []
var former_temperature = widgetProps.FormerTemparature || []
var plc_reading = widgetProps.PlcReading || []

var chemical_mix_rm_number = widgetProps.ChemicalMixRmNumber || []


// batas page1


//page 2 
var configuration_page2 = widgetProps.ConfigurationConcentrationFlowmeter || []


var data_from_thingworx_page2_concentration = widgetProps.DataTableConcentration || []

var data_from_thingworx_page2_flowmeter = widgetProps.DataTableFlowmeter || []

configuration_page2 = configuration_page2.slice(0,11)
var header_column_page2 = [];


configuration_page2.forEach((element) => {
    header_column_page2.push(element.header_name);
});

var datatable_page2 = [];

var  data_from_thingworx_page2 = []

data_from_thingworx_page2_concentration.forEach((element,i)=>{

    data_from_thingworx_page2[i] = { ...data_from_thingworx_page2_concentration[i],...data_from_thingworx_page2_flowmeter[i]}
})

data_from_thingworx_page2.forEach((elementi, i) => {
    var tempdatatable = [];

    configuration_page2.forEach((elementj, j) => {
        var tempelemnt = null;

        if (elementi[elementj.data_shape_column_name]) {
            tempelemnt = elementi[elementj.data_shape_column_name];
        }

        tempdatatable.push(tempelemnt);
    });

    datatable_page2.push(tempdatatable);
});

//batas page 2

//p input age 3
var data_from_thingworx_latex_information =   widgetProps.DataTableLatexInformation || []

var configuration_latex_information = widgetProps.ConfigurationLatexInformation || []  

var data_from_thingworx_dipping_temperature= widgetProps.DataTableDippingTemperature || []  

var configuration_dipping_temperature = widgetProps.ConfigurationDippingTemperature || [] 

var data_from_thingworx_downtime = widgetProps.DataTableDowntime || [] 

var configuration_downtime = widgetProps.ConfigurationDowntime || [] 





var elementchild_latex_information=[[]]


var countrarray = 0
var sumcount = 0
var arraytemp = []
configuration_latex_information.forEach((element,i)=>{
    sumcount +=1
    arraytemp.push(element)
    if(sumcount == 15 || i == configuration_latex_information.length - 1){
        elementchild_latex_information[countrarray]= arraytemp
        arraytemp = []
         sumcount = 0
        countrarray += 1
    }

})

var elementchild_dipping_temperature=[[]]


var countrarray = 0
var sumcount = 0
var arraytemp = []
configuration_dipping_temperature.forEach((element,i)=>{
    sumcount +=1
    arraytemp.push(element)
    if(sumcount == 15 || i == configuration_dipping_temperature.length - 1){
        elementchild_dipping_temperature[countrarray]= arraytemp
        arraytemp = []
         sumcount = 0
        countrarray += 1
    }

})

var elementchild_downtime=[[]]


var countrarray = 0
var sumcount = 0
var arraytemp = []
configuration_downtime.forEach((element,i)=>{
    sumcount +=1
    arraytemp.push(element)
    if(sumcount == 15 || i == configuration_downtime.length - 1){
        elementchild_downtime[countrarray]= arraytemp
        arraytemp = []
         sumcount = 0
        countrarray += 1
    }

})


// batas page 3
//

configuration_width = configuration_width.slice(0,16)
var colgroupdata = [
    { name: "colum1", width: "161" },
    { name: "colum2", width: "84" },
    { name: "colum3", width: "64" },
    { name: "colum4", width: "94" },
    { name: "colum5", width: "84" },
    { name: "colum6", width: "64" },
    { name: "colum7", width: "124" },
    { name: "colum8", width: "64" },
    { name: "colum9", width: "64" },
    { name: "colum10", width: "104" },
    { name: "colum11", width: "134" },
    { name: "colum12", width: "64" },
    { name: "colum13", width: "164" },
    { name: "colum14", width: "84" }, 
    { name: "colum15", width: "64" },
    //change here
];

configuration_width.forEach((element, i) => {
    colgroupdata[i] = {
        name: "element.data_shape_column_name",
        width: element.width,
    };
});

colgroupdata.push({ name: "colum16", width: "64" });

console.log("colgroupdata")
console.log(colgroupdata)

var imageexport = d3
    .select(`#${widgetID}`)
    .append("img")
    .style("cursor", "pointer")
    .style("display", function () {
        if (booleandisplaybutton) {
            return "";
        } else {
            return "none";
        }
    })
    .attr(
        "src",
        "data:image/png;base64,"
    )
    .attr("id", "buttonimage" + idRandom)
    .attr("alt", "button-export")
    .attr("width", widhtimage + "px")
    .attr("height", heightimage + "px")
    .attr("title", function () {
        // if (booleanpdf && booleanexcell) {
        //     return "Export to PDF & Excel";
        // } else if (!booleanpdf && !booleanexcell) {
        //     return "Select Type Export";
        // } else if (!booleanexcell) {
        return "Export to PDF";
        // } else {
        //     return "Export to Excel";
        // }
    })
    .on("click", function (d, i) {
        // if (booleanpdf && booleanexcell) {
        // converHTMLToCanvas("testTable" + idRandom, file_name);
        converHTMLToCanvaspage1("page" + (1) +idRandom , file_name);
        converHTMLToCanvaspage2("page" + (2) +idRandom , file_name);
        converHTMLToCanvaspage345("latexinformaton"+idRandom, file_name ,elementchild_latex_information,"LATEX INFORMATION",data_from_thingworx_latex_information );
            converHTMLToCanvaspage345("dippingtemperature"+idRandom, file_name ,elementchild_dipping_temperature,"DIPPING TEMPERATURE",data_from_thingworx_dipping_temperature);
            converHTMLToCanvaspage345("downtime"+idRandom, file_name ,elementchild_downtime,"DOWNTIME",data_from_thingworx_downtime ,true);
        
        // } else if (!booleanpdf && !booleanexcell) {
        // } else if (!booleanexcell) {
        //     converHTMLToCanvas("testTable"+idRandom, file_name);
        // } else {
        //     tableToExcel("testTable"+idRandom, "worksheet", file_name);
        // }
    });

var divbody = d3
    .select(`#${widgetID}`)
    .append("div")
    .attr("id", "root" + idRandom);

var elementold = "";

async function exportpdfnow(width, file_name) {
    var element = document.getElementById("root" + idRandom);

    var optionArray = {
        margin: 6,
        filename: file_name + ".pdf",
        pageBreak: { mode: "css", before: "#nextpage1" },
        image: { type: "png", quality: 0.1 },
        html2canvas: { scale: 0.9, width: width + 12, dpi: 192 },
        jsPDF: { unit: "pt", format: "a4", orientation: "landscape" },
    };

    // html to pdf generation with the reference of PDF worker object

    html2pdf().set(optionArray).from(element).save();
}

async function remove(page1, allpage) {


    var page = page1;
    while (page <= allpage) {
        d3.selectAll("#" + ("page" + page + idRandom)).remove();

        page++;
    }
}

 

async function editpagetext(page1, allpage) {

    var page = page1;
    while (page <= allpage) {
   
        d3.select("#Pagepage" + (page) +idRandom ).text("Page " + page + " of " + allpage);
        page++;
    }
}


var addpage = 1
var addmarginbottom = 1 ;


// code page1

function makeTablepage1(idtable) {
    var table = divbody
        .append("table")
        .attr("cellspacing", "0")
        .attr("id", idtable)
        .style("box-sizing", "border-box")
        .style("border-collapse", "collapse")
        .style("table-layout", "fixed")
        .style("width", "fit-content");

    table
        .selectAll("colgroup")
        .data(colgroupdata)
        .enter()
        .append("colgroup")

        .attr("width", function (d, i) {
            return d.width;
        });

    var tbody = table.append("tbody");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000").attr("id", "header_table" + idtable);
    rowheader1
        .append("td")
        .attr("rowspan", function () {
            if (doc_information.length > 0) return doc_information.length;
            return 1;
        })
        .attr("id", "image_hansell")
        .attr("height", "141")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("vertical-align", "middle")
        .style("font-size", "30px")
        .style("font-weight", "bold")
        .style("font-family", "Arial")
        .style("color", "#000000")

        .text("")
        .append("img")
        .attr("id", "image_hansell_img" + idtable)
        .attr("src", "");

    rowheader1
        .append("td")
        .attr("rowspan", function () {
            if (doc_information.length > 0) return doc_information.length;
            return 1;
        })
        .attr("colspan", colgroupdata.length - 7) //change here
        .style("border", "1px solid #000000")
        .style("vertical-align", "middle")
        .style("font-weight", "bold")
        .style("text-align", "center")
        .style("font-size", "30px")
        .style("font-family", "Arial")
        .style("color", "#000000")
        // .style("text-transform", "uppercase")

        .text(header_name);
    if (doc_information.length == 0) {
        d3.select("#header_table" + idtable)
            .append("td")
            .attr("colspan", "3")
            .style("border", "1px solid #000000")
            .style("vertical-align", "middle")
            .style("text-align", "left")
            .style("font-size", "18px")
            .style("font-family", "Arial")
            .style("color", "#000000")
            .text("");

        d3.select("#header_table" + idtable)
            .append("td")
            .attr("colspan", "3")
            .style("border", "1px solid #000000")
            .style("vertical-align", "middle")
            .style("text-align", "left")
            .style("font-size", "18px")
            .style("font-family", "Arial")
            .style("color", "#000000")
            .text("");
    } else {
        doc_information.forEach((element, i) => {
            if (i == 0) {
                // tr1

                d3.select("#header_table" + idtable)
                    .append("td")
                    .attr("colspan", "3") //change here

                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.name);

                d3.select("#header_table" + idtable)
                    .append("td")
                    .attr("colspan", "3")
                    .attr("id", element.name + idtable)
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.value);
            } else {
                var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
                rowheader1
                    .append("td")
                    .attr("colspan", "3") //change here
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.name);
                rowheader1
                    .append("td")
                    .attr("colspan", "3")
                    .attr("id", element.name + idtable)
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.value);
            }
        });
    }

    var row_header_information = Math.ceil(header_information.length / 5);
    var count_row_header_information = 1;

    //header_information
    while (count_row_header_information <= row_header_information) {
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        var countcolumn = 0;

        header_information
            .slice(
                0 + (count_row_header_information - 1) * 5,
                5 + (count_row_header_information - 1) * 5
            )
            .forEach((element_header_information, i_header_information) => {
                countcolumn += 3;
                rowheader1
                    .append("td")
                    .attr("height", "21")
                    .attr("id", "header_information")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-left", function () {
                        if (i_header_information == 0) {
                            return "1px solid #000000";
                        }
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")

                    .text(element_header_information.name + " :");

                rowheader1
                    .append("td")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    // .style("background-color", "#FFFF00")

                    .text(element_header_information.value);

                // if(i_header_information !== 4) {
                rowheader1
                    .append("td")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
                // }
            });

        countcolumn++;
        while (countcolumn <= colgroupdata.length) {
            if (countcolumn !== colgroupdata.length) {
                rowheader1
                    .append("td")
                    .attr("id", "unoquie")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else {
                rowheader1
                    .append("td")
                    .attr("id", "unoquie end")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-right", "1px solid #000000")
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            }
            countcolumn++;
        }

        if (count_row_header_information !== row_header_information) {
            var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
            for (let i = 0; i < colgroupdata.length; i++) {
                if (i == 0) {
                    rowheader1
                        .append("td")
                        .attr("height", "21")
                        .style("border-left", "1px solid #000000")
                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text("");
                } else if (i == colgroupdata.length - 1) {
                    rowheader1
                        .append("td")

                        .style("border-right", "1px solid #000000")

                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000");
                } else {
                    rowheader1
                        .append("td")
                        // .style("border-top", "1px solid #000000")

                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text("");
                }
            }
        }

        count_row_header_information++;
    }

    //dashboar_information

    var row_dashboard_information = Math.ceil(dashboard_information.length / 5);
    var count_row_dashboard_information = 1;

    while (count_row_dashboard_information <= row_dashboard_information) {
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        for (let i = 0; i < colgroupdata.length; i++) {
            if (i == 0) {
                rowheader1
                    .append("td")
                    .attr("height", "21")
                    .style("border-left", "1px solid #000000")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else if (i == colgroupdata.length - 1) {
                rowheader1
                    .append("td")

                    .style("border-right", "1px solid #000000")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            } else {
                rowheader1
                    .append("td")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })

                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            }
        }
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        var countcolumn = 0;

        dashboard_information
            .slice(
                0 + (count_row_dashboard_information - 1) * 5,
                5 + (count_row_dashboard_information - 1) * 5
            )
            .forEach((element_header_information, i_header_information) => {
                countcolumn += 3;
                rowheader1
                    .append("td")
                    .attr("height", "21")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-left", function () {
                        if (i_header_information == 0) {
                            return "1px solid #000000";
                        }
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text(element_header_information.name + " :");

                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    // .style("background-color", "#FFFF00")
                    .text(element_header_information.value);

                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            });

        countcolumn++;
        while (countcolumn <= colgroupdata.length) {
            if (countcolumn !== colgroupdata.length) {
                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else {
                rowheader1
                    .append("td")

                    .style("border-right", "1px solid #000000")
                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            }
            countcolumn++;
        }

        count_row_dashboard_information++;
    }

    //data table
    //row8
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");

    rowheader1
        .append("td")
        .attr("height", "20")
        .attr("colspan", "6")
        .style("border", "1px solid #000000")

        .style("text-align", "center")
        .style("vertical-align", "bottom")
        .style("font-family", "Arial")
        .style("font-size", "16px")
        .style("font-weight", "bold")
        .style("color", "#000000")
        .text("LABOUR PRODUCTIVITY");
    rowheader1
        .append("td")
        .attr("height", "21")
        .attr("colspan", "5")
        .style("border", "1px solid #000000")

        .style("text-align", "center")
        .style("vertical-align", "bottom")
        .style("font-family", "Arial")
        .style("font-size", "16px")
        .style("font-weight", "bold")
        .style("color", "#000000")
        .text("FORMER TEMPERATURE");

    rowheader1
        .append("td")
        .attr("height", "21")
        .attr("colspan", "5")
        .style("border", "1px solid #000000")

        .style("text-align", "center")
        .style("vertical-align", "bottom")
        .style("font-family", "Arial")
        .style("font-size", "16px")
        .style("font-weight", "bold")
        .style("color", "#000000")
        .text("Chemical Mix/RM Number");

 

    //batas infotable

    if (labour_productivity.length < 1) {
        labour_productivity.push({
            name: "",
            value: "",
            unit: "",
        });
    }
    var plus_minus_labour_productivity = 0;

    if (labour_productivity.length > 1) {
        var plus_minus_labour_productivity = labour_productivity.length - 1;
    }

    if (former_temperature.length < 2) {
        for (let index = former_temperature.length; index < 2; index++) {
            former_temperature.push({
                name: "",
                before_dope_palm: "",
                before_dope_cuff: "",
                before_latex_palm: "",
                before_latex_cuff: "",
            });
        }
    }
    var additional_former_temperature = 0;
    if (former_temperature.length > 2) {
        additional_former_temperature = former_temperature.length - 2;
    }

    var max1 = 4 + plus_minus_labour_productivity + dds_information.length;
    var max2 = 4 + plus_minus_labour_productivity + loading_schedule.length;
    var max3 = 7 + additional_former_temperature + plc_reading.length;
    var max4 = 1 + chemical_mix_rm_number.length;

    // //console.log(max2);

    var max_row = Math.max(13, max1, max2, max3, max4);

    // //console.log("max_row");
    // //console.log(max_row);

    if (
        dds_information.length <
        max_row - (4 + plus_minus_labour_productivity)
    ) {
        for (
            let index = dds_information.length;
            index < max_row - (4 + plus_minus_labour_productivity);
            index++
        ) {
            dds_information.push({
                name: "",
                value: "",
            });
        }
    }

    if (
        loading_schedule.length <
        max_row - (4 + plus_minus_labour_productivity)
    ) {
        for (
            let index = loading_schedule.length;
            index < max_row - (4 + plus_minus_labour_productivity);
            index++
        ) {
            loading_schedule.push({
                size: "",
                type: "",
                loading_schedule: "",
                persentase: "",
            });
        }
    }

    if (plc_reading.length < max_row - (7 + additional_former_temperature)) {
        for (
            let index = plc_reading.length;
            index < max_row - (7 + additional_former_temperature);
            index++
        ) {
            plc_reading.push({
                station: "",
                spec: "",
                actual: "",
            });
        }
    }

    if (chemical_mix_rm_number.length < max_row - 1)
        var countwhile = chemical_mix_rm_number.length;
    countwhile++;
    while (countwhile <= max_row - 1) {
        chemical_mix_rm_number.push({
            name: "",
            value: "",
        });
        countwhile++;
    }

    var labour_productivity_new = [[]];
    labour_productivity.forEach((element, i) => {
        labour_productivity_new[i] = [
            {
                order: i + 1,
                height: "20",
                colspan: "2",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.name,
            },
            {
                order: i + 1,
                height: "20",
                colspan: "2",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.value,
            },
            {
                order: i + 1,
                height: "20",
                colspan: "2",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.unit,
            },
        ];
    });

    // //console.log("labour_productivity_new");
    // //console.log(labour_productivity_new);

    var rowtable_two_three_ddsinformation_loading = [[]];

    rowtable_two_three_ddsinformation_loading[
        1 + plus_minus_labour_productivity
    ] = [
        {
            order: 2,
            height: "40",
            colspan: "2",
            rowspan: "2",
            text_align: "center",
            font_weight: "bold",
            text: "DDS INFORMATION",
        },
        {
            order: 2,
            height: "40",
            colspan: "4",
            rowspan: "2",
            text_align: "center",
            font_weight: "bold",
            text: "LOADING SCHEDULE",
        },
    ];
    rowtable_two_three_ddsinformation_loading[
        3 + plus_minus_labour_productivity
    ] = [
        {
            order: 4,
            height: "20",
            colspan: "1",
            rowspan: "1",
            text_align: "center",
            font_weight: "",
            text: "Size",
        },
        {
            order: 4,
            height: "20",
            colspan: "1",
            rowspan: "1",
            text_align: "center",
            font_weight: "",
            text: "Type",
        },
        {
            order: 4,
            height: "20",
            colspan: "1",
            rowspan: "1",
            text_align: "center",
            font_weight: "",
            text: "Loading Schedule",
        },
        {
            order: 4,
            height: "20",
            colspan: "1",
            rowspan: "1",
            text_align: "center",
            font_weight: "",
            text: "%",
        },
    ];

    // //console.log("rowtable_two_three_ddsinformation_loading");
    // //console.log(rowtable_two_three_ddsinformation_loading);

    var dds_information_new = [[]];

    dds_information.forEach((element, i) => {
        if (i == 0) {
            dds_information_new[i + 3 + plus_minus_labour_productivity] = [
                {
                    order: i + 4 + plus_minus_labour_productivity,
                    height: "40",
                    colspan: "1",
                    rowspan: "2",
                    text_align: "center",
                    font_weight: "",
                    text: element.name,
                },
                {
                    order: i + 4 + plus_minus_labour_productivity,
                    height: "40",
                    colspan: "1",
                    rowspan: "2",
                    text_align: "center",
                    font_weight: "",
                    text: element.value,
                },
            ];
        } else {
            dds_information_new[i + 4 + plus_minus_labour_productivity] = [
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "1",
                    rowspan: "1",
                    text_align: "left",
                    font_weight: "",
                    text: element.name,
                },
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "1",
                    rowspan: "1",
                    text_align: "center",
                    font_weight: "",
                    text: element.value,
                },
            ];
        }
    });

    // //console.log("dds_information_new");
    // //console.log(dds_information_new);

    var loading_schedule_new = [[]];

    loading_schedule.forEach((element, i) => {
        if (element.type == "Total") {
            loading_schedule_new[i + 4 + plus_minus_labour_productivity] = [
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "2",
                    rowspan: "1",
                    text_align: "center",
                    font_weight: "bold",
                    text: element.type,
                },
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "1",
                    rowspan: "1",
                    text_align: "center",
                    font_weight: "",
                    text: element.loading_schedule,
                },
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "1",
                    rowspan: "1",
                    text_align: "center",
                    font_weight: "",
                    text: element.persentase,
                },
            ];
        } else {
            loading_schedule_new[i + 4 + plus_minus_labour_productivity] = [
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "1",
                    rowspan: "1",
                    text_align: "center",
                    font_weight: "",
                    text: element.size,
                },
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "1",
                    rowspan: "1",
                    text_align: "center",
                    font_weight: "",
                    text: element.type,
                },
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "1",
                    rowspan: "1",
                    text_align: "center",
                    font_weight: "",
                    text: element.loading_schedule,
                },
                {
                    order: i + 5 + plus_minus_labour_productivity,
                    height: "20",
                    colspan: "1",
                    rowspan: "1",
                    text_align: "center",
                    font_weight: "",
                    text: element.persentase,
                },
            ];
        }
    });

    // //console.log("loading_schedule_new");
    // //console.log(loading_schedule_new);

    var rowtable_one_and_two_former_temperature = [
        [
            {
                order: 1,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "",
            },
            {
                order: 1,
                height: "20",
                colspan: "2",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "Before Dope",
            },
            {
                order: 1,
                height: "20",
                colspan: "2",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "Before Latex",
            },
            {
                order: 1,
                height: "20",
                colspan: "2",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "Chemical",
            },
            {
                order: 1,
                height: "20",
                colspan: "3",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "Mix/RM Number",
            },
        ],
        [
            {
                order: 2,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "",
            },
            {
                order: 2,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "Palm",
            },
            {
                order: 2,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "Cuff",
            },
            {
                order: 2,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "Palm",
            },
            {
                order: 2,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: "Cuff",
            },
        ],
    ];

    // //console.log("rowtable_one_and_two_former_temperature");
    // //console.log(rowtable_one_and_two_former_temperature);

    var former_temperature_new = [[]];

    former_temperature.forEach((element, i) => {
        former_temperature_new[i + 2] = [
            {
                order: i + 3,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.name,
            },
            {
                order: i + 3,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.before_dope_palm,
            },
            {
                order: i + 3,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.before_dope_cuff,
            },
            {
                order: i + 3,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.before_latex_palm,
            },
            {
                order: 3,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.before_latex_cuff,
            },
        ];
    });

    // //console.log("former_temperature_new");
    // //console.log(former_temperature_new);

    var rowtable_five_seven_plcreading = [[]];
    rowtable_five_seven_plcreading[4 + additional_former_temperature] = [
        {
            order: 5,
            height: "40",
            colspan: "5",
            rowspan: "2",
            text_align: "center",
            font_weight: "bold",
            text: "PLC Reading",
        },
    ];
    rowtable_five_seven_plcreading[6 + additional_former_temperature] = [
        {
            order: 7,
            height: "20",
            colspan: "3",
            rowspan: "1",
            text_align: "center",
            font_weight: "",
            text: "Station",
        },
        {
            order: 7,
            height: "20",
            colspan: "1",
            rowspan: "1",
            text_align: "center",
            font_weight: "",
            text: "Spec",
        },
        {
            order: 7,
            height: "20",
            colspan: "1",
            rowspan: "1",
            text_align: "center",
            font_weight: "",
            text: "Actual",
        },
    ];

    // //console.log("rowtable_five_seven_plcreading");
    // //console.log(rowtable_five_seven_plcreading);

    var plc_reading_new = [[]];

    plc_reading.forEach((element, i) => {
        plc_reading_new[i + 7 + additional_former_temperature] = [
            {
                order: i + 8 + additional_former_temperature,
                height: "20",
                colspan: "3",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.station,
            },
            {
                order: i + 8 + additional_former_temperature,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.spec,
            },
            {
                order: i + 8 + additional_former_temperature,
                height: "20",
                colspan: "1",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.actual,
            },
        ];
    });

    // //console.log("plc_reading_new");
    // //console.log(plc_reading_new);

    var chemical_mix_rm_number_new = [[]];

    chemical_mix_rm_number.forEach((element, i) => {
        chemical_mix_rm_number_new[i + 1] = [
            {
                order: i + 2,
                height: "20",
                colspan: "2",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.name,
            },
            {
                order: i + 2,
                height: "20",
                colspan: "3",
                rowspan: "1",
                text_align: "center",
                font_weight: "",
                text: element.value,
            },
        ];
    });

    // //console.log("chemical_mix_rm_number_new");
    // //console.log(chemical_mix_rm_number_new);

    var datatablenewtemp = [[]];

    var count_datatablenewtemp = 0;

    while (count_datatablenewtemp < max_row) {
        if (count_datatablenewtemp < max_row) {
            datatablenewtemp[count_datatablenewtemp] = [
                ...(labour_productivity_new[count_datatablenewtemp] || []),
                ...(dds_information_new[count_datatablenewtemp] || []),
                ...(rowtable_two_three_ddsinformation_loading[
                    count_datatablenewtemp
                ] || []),
                ...(loading_schedule_new[count_datatablenewtemp] || []),
                ...(rowtable_one_and_two_former_temperature[
                    count_datatablenewtemp
                ] || []),
                ...(former_temperature_new[count_datatablenewtemp] || []),
                ...(rowtable_five_seven_plcreading[count_datatablenewtemp] ||
                    []),
                ...(plc_reading_new[count_datatablenewtemp] || []),
                ...(chemical_mix_rm_number_new[count_datatablenewtemp] || []),
            ];
        }

        count_datatablenewtemp++;
    }

    // //console.log("datatablenewtemp");
    // //console.log(datatablenewtemp);

  
    datatablenewtemp.forEach((element, i) => {
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        element.forEach((elementchild, j) => {
            rowheader1
                .append("td")
                .attr("height", elementchild.height)

                .attr("colspan", elementchild.colspan)
                .attr("rowspan", elementchild.rowspan)
                .style("border", "1px solid #000000")

                .style("text-align", elementchild.text_align)
                .style("vertical-align", "center")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("font-weight", elementchild.font_weight)
                .style("color", "#000000")
                .text(elementchild.text);
        });
    });

    var tbody = table.append("tbody").attr("id", "tabody_data" + idtable);

    var tbody = table.append("tbody");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    rowheader1
        .append("td")
        .attr("id", "image_buttom")
        .attr("rowspan", "2")
        .attr("colspan", colgroupdata.length)
        .attr("height", "40")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("border-width", "1px 1px 0 1px")
        .style("vertical-align", "bottom")
        .text("")
        .append("img")
        .attr("id", "img_image_button" + idtable)
        .attr("src", "");

    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    rowheader1
        .append("td")

        .attr("colspan", colgroupdata.length)
        .attr("height", "30")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("border-width", "0 1px 1px 1px")
        .style("font-style", "italic")
        .style("vertical-align", "center")
        .text("This document is generated from AIMS");

    d3.select("#image_hansell_img" + idtable)
        .attr(
            "src",
            "data:image/png;base64,"
        )
        .attr("width", "119")
        .attr("height", "73")
        .attr("hspace", "21")
        .attr("vspace", "22");

    d3.select("#img_image_button" + idtable)
        .attr(
            "src",
            "data:image/png;base64,"
        )
        .attr("width", "442")
        .attr("height", "25")
        .attr("hspace", "185")
        .attr("vspace", "8");
}

async function converHTMLToCanvaspage1(elementold, file_name) {
  
    makeTablepage1(elementold);
    var width = d3
        .select("#" + elementold)
        .node()
        .getBoundingClientRect().width;

    // var height = $(element).height();
    var height = d3
        .select("#" + elementold)
        .node()
        .getBoundingClientRect().height;

    //console.log("widht", width, "height", height);

    var sizeheightpdfmax = (916 / 1302.015625) * width;


    d3.select("#" + elementold).style(
        "margin-bottom",
        ((sizeheightpdfmax - height) +6 +addmarginbottom ) + "px"
    );
  
        // await exportpdfnow(width, file_name);
        // await remove(1,2);
    
}






// code page2




function makeTablepage2(idtable) {
    var table = divbody
        .append("table")
        .attr("cellspacing", "0")
        .attr("border", "0")
        .attr("id", idtable)
   
        .style("box-sizing", "border-box")
        .style("border-collapse", "collapse")
        .style("table-layout", "fixed")
        .style("width", "fit-content")
        
       


    table
        .selectAll("colgroup")
        .data(colgroupdata)
        .enter()
        .append("colgroup")

        .attr("width", function (d, i) {
            return d.width;
        });

    var tbody = table.append("tbody");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000").attr("id", "header_table" + idtable);
    rowheader1
        .append("td")
        .attr("rowspan", function () {
            if (doc_information.length > 0) return doc_information.length;
            return 1;
        })
        .attr("id", "image_hansell")
        .attr("height", "141")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("vertical-align", "middle")
        .style("font-size", "30px")
        .style("font-weight", "bold")
        .style("font-family", "Arial")
        .style("color", "#000000")

        .text("")
        .append("img")
        .attr("id", "image_hansell_img" + idtable)
        .attr("src", "");

    rowheader1
        .append("td")
        .attr("rowspan", function () {
            if (doc_information.length > 0) return doc_information.length;
            return 1;
        })
        .attr("colspan", colgroupdata.length - 7) //change here
        .style("border", "1px solid #000000")
        .style("vertical-align", "middle")
        .style("font-weight", "bold")
        .style("text-align", "center")
        .style("font-size", "30px")
        .style("font-family", "Arial")
        .style("color", "#000000")
        // .style("text-transform", "uppercase")

        .text(header_name);
    if (doc_information.length == 0) {
        d3.select("#header_table" + idtable)
            .append("td")
            .attr("colspan", "3")
            .style("border", "1px solid #000000")
            .style("vertical-align", "middle")
            .style("text-align", "left")
            .style("font-size", "18px")
            .style("font-family", "Arial")
            .style("color", "#000000")
            .text("");

        d3.select("#header_table" + idtable)
            .append("td")
            .attr("colspan", "3")
            .style("border", "1px solid #000000")
            .style("vertical-align", "middle")
            .style("text-align", "left")
            .style("font-size", "18px")
            .style("font-family", "Arial")
            .style("color", "#000000")
            .text("");
    } else {
        doc_information.forEach((element, i) => {
            if (i == 0) {
                // tr1

                d3.select("#header_table" + idtable)
                    .append("td")
                    .attr("colspan", "3") //change here

                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.name);

                d3.select("#header_table" + idtable)
                    .append("td")
                    .attr("colspan", "3")
                    .attr("id", element.name + idtable)
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.value);
            } else {
                var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
                rowheader1
                    .append("td")
                    .attr("colspan", "3") //change here
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.name);
                rowheader1
                    .append("td")
                    .attr("colspan", "3")
                    .attr("id", element.name + idtable)
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.value);
            }
        });
    }

    var row_header_information = Math.ceil(header_information.length / 5);
    var count_row_header_information = 1;

    //header_information
    while (count_row_header_information <= row_header_information) {
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        var countcolumn = 0;

        header_information
            .slice(
                0 + (count_row_header_information - 1) * 5,
                5 + (count_row_header_information - 1) * 5
            )
            .forEach((element_header_information, i_header_information) => {
        
                countcolumn += 3;
                rowheader1
                    .append("td")
                    .attr("height", "21")
                    .attr("id","header_information")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-left", function () {
                        if (i_header_information == 0) {
                            return "1px solid #000000";
                        }
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")

                    .text(element_header_information.name + " :");

                rowheader1
                    .append("td")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    // .style("background-color", "#FFFF00")

                    .text(element_header_information.value);

                    // if(i_header_information !== 4) {
                        rowheader1
                        .append("td")
                        .style("border-top", function () {
                            if (count_row_header_information == 1)
                                return "1px solid #000000";
                            return "";
                        })
                        .style("border-bottom", function () {
                            if (
                                count_row_header_information ==
                                row_header_information
                            )
                                return "1px solid #000000";
                            return "";
                        })
                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text("");
                    // }
                   
        
            });

        countcolumn++;
        while (countcolumn <= colgroupdata.length) {
            if (countcolumn !== colgroupdata.length) {
                rowheader1
                    .append("td")
                    .attr("id","unoquie")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else {
                rowheader1
                    .append("td")
                    .attr("id","unoquie end")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-right", "1px solid #000000")
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            }
            countcolumn++;
        }

        if (count_row_header_information !== row_header_information) {
            var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
            for (let i = 0; i < colgroupdata.length; i++) {
                if (i == 0) {
                    rowheader1
                        .append("td")
                        .attr("height", "21")
                        .style("border-left", "1px solid #000000")
                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text("");
                } else if (i == colgroupdata.length - 1) {
                    rowheader1
                        .append("td")

                        .style("border-right", "1px solid #000000")

                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000");
                } else {
                    rowheader1
                        .append("td")
                        // .style("border-top", "1px solid #000000")

                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text("");
                }
            }
        }

        count_row_header_information++;
    }

    //dashboar_information

    var row_dashboard_information = Math.ceil(dashboard_information.length / 5);
    var count_row_dashboard_information = 1;

    while (count_row_dashboard_information <= row_dashboard_information) {
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        for (let i = 0; i < colgroupdata.length; i++) {
            if (i == 0) {
                rowheader1
                    .append("td")
                    .attr("height", "21")
                    .style("border-left", "1px solid #000000")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else if (i == colgroupdata.length - 1) {
                rowheader1
                    .append("td")

                    .style("border-right", "1px solid #000000")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            } else {
                rowheader1
                    .append("td")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })

                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            }
        }
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        var countcolumn = 0;

        dashboard_information
            .slice(
                0 + (count_row_dashboard_information - 1) * 5,
                5 + (count_row_dashboard_information - 1) * 5
            )
            .forEach((element_header_information, i_header_information) => {
                countcolumn += 3;
                rowheader1
                    .append("td")
                    .attr("height", "21")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-left", function () {
                        if (i_header_information == 0) {
                            return "1px solid #000000";
                        }
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text(element_header_information.name + " :");

                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    // .style("background-color", "#FFFF00")
                    .text(element_header_information.value);

                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            });

        countcolumn++;
        while (countcolumn <= colgroupdata.length) {
            if (countcolumn !== colgroupdata.length) {
                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else {
                rowheader1
                    .append("td")

                    .style("border-right", "1px solid #000000")
                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            }
            countcolumn++;
        }

        count_row_dashboard_information++;
    }



    //data table
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    // for (let i = 0; i < colgroupdata.length; i++) {
    //     if (i == 0) {
            rowheader1
                .append("td")
                .attr("height", "21")
                .attr("colspan", "11")
                .style("border", "1px solid #000000")

                .style("text-align", "center")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("font-weight","bold")
                .style("color", "#000000")
                .text("CONCENTRATION");
                rowheader1
                .append("td")
                .attr("height", "21")
                .attr("colspan", "5")
                .style("border", "1px solid #000000")

                .style("text-align", "center")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("font-weight","bold")
                .style("color", "#000000")
                .text("FLOW-RATES");

            
      

 

  

    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000")
    for (let i = 0; i < colgroupdata.length; i++) {
        if (i == 0) {
            rowheader1
                .append("td")
                .attr("height", "21")
                .attr("colspan", "4")
                .style("border", function () {
                    if (header_column_page2[i ]) {
                        return "1px solid #000000";
                    }
                    return "";
                })

                .style("text-align", "center")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000")
                .text(function () {
                    return header_column_page2[i ];
                });
        } else if (i == colgroupdata.length - 1) {
            // rowheader1
            //     .append("td")

            //     .style("border-right", "1px solid #000000")

            //     .style("text-align", "left")
            //     .style("vertical-align", "bottom")
            //     .style("font-family", "Arial")
            //     .style("font-size", "16px")
            //     .style("color", "#000000");
        } else {

            if ( i >=2){
       
                rowheader1
                .append("td")
                .attr("colspan", function () {

                    if (i == header_column_page2.length ){

                        return "3"
                    }else {
                        return "1"
                    }
                   
                  
                } )
                .style("border", function () {
                    if (header_column_page2[i - 1]) {
                        return "1px solid #000000";
                    }
                    return "";
                })
                .style("text-align", "center")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000")

                .text(function () {
                    return header_column_page2[i - 1];
                });
            }


           
        }
    }

    var tbody = table.append("tbody").attr("id", "tabody_data" + idtable);

    var tbody = table.append("tbody");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    rowheader1
        .append("td")
        .attr("id", "image_buttom")
        .attr("rowspan", "2")
        .attr("colspan", colgroupdata.length)
        .attr("height", "40")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("border-width", "1px 1px 0 1px")
        .style("vertical-align", "bottom")
        .text("")
        .append("img")
        .attr("id", "img_image_button" + idtable)
        .attr("src", "");

    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    rowheader1
        .append("td")

        .attr("colspan", colgroupdata.length)
        .attr("height", "30")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("border-width", "0 1px 1px 1px")
        .style("font-style", "italic")
        .style("vertical-align", "center")
        .text("This document is generated from AIMS");

        d3.select("#image_hansell_img" + elementold)
                            .attr(
                                "src",
                                "data:image/png;base64,"
                            )
                            .attr("width", "119")
                            .attr("height", "73")
                            .attr("hspace", "21")
                            .attr("vspace", "22");

                            d3.select("#img_image_button" + elementold)
                            .attr(
                                "src",
                                "data:image/png;base64,"
                            )
                            .attr("width", "442")
                            .attr("height", "25")
                            .attr("hspace", "185")
                            .attr("vspace", "8");
}


async function converHTMLToCanvaspage2(  elementold, file_name) {
       

       

       makeTablepage2(elementold);
      

    
       
    
        var sum_count_row_data = datatable_page2.length ;

        var count_row_data = 1;
        while (count_row_data <= sum_count_row_data) {
            var rowheader1 = d3
                .select("#tabody_data" + elementold)
                .append("tr").style("border-right","1px solid #000")
                .attr("id", "row_data_table" + elementold)
                .style("border-right","1px solid #000")
                .attr("class", "row_data_table" + elementold);
            for (let i = 0; i < colgroupdata.length; i++) {
                if (i == 0) {
                    rowheader1
                        .append("td")
                        .attr("height", "21")
                        .attr("colspan", "4")
                        .style("border-bottom", "1px solid #000000")
                        .style("border-left", "1px solid #000000")
                        .style("text-align", "center")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text(function () {
                           
                                return datatable_page2[count_row_data - 1][i];
                            
                         
                        });
                } else if (i == colgroupdata.length - 1) {
                    // rowheader1
                    //     .append("td")
    
                    //     .style("border-right", "1px solid #000000")
    
                    //     .style("text-align", "left")
                    //     .style("vertical-align", "bottom")
                    //     .style("font-family", "Arial")
                    //     .style("font-size", "16px")
                    //     .style("color", "#000000");
                } else {
                  
                    if ( i >=2){
                        rowheader1
                        .append("td")
                        .attr("colspan", function () {
        
                            if (i == header_column_page2.length ){
        
                                return "3"
                            }else {
                                return "1"
                            }
                           
                          
                        } )
                        .style("border", function () {
                            if (datatable_page2[count_row_data - 1]) {
                                if (
                                    datatable_page2[count_row_data - 1][i - 1] !==
                                    undefined
                                )
                                    return "1px solid #000000";
                            }
                            return "";
                        })
                        .style("text-align", "center")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text(function () {
                            if (datatable_page2[count_row_data - 1]) {
                                return datatable_page2[count_row_data - 1][i - 1];
                            }
                            return "";
                        });
    
                    }
                   
                }
            }
            count_row_data++;
        }
    
        var jspdf = new jsPDF("l", "pt", "a4", true);
    
        // var width = $(element).width() ;
        var width = d3.select("#" + elementold)
            .node()
            .getBoundingClientRect().width;
    
        // var height = $(element).height();
        var height = d3.select("#" + elementold)
            .node()
            .getBoundingClientRect().height;
    
        
            // console.log("widht", width , "height" ,height)
    
      
    
        var sizeheightpdfmax = ((916 ) / 1302.015625) * width;
    
      
    
        var sizeheightpdf = height;
    

        var sizeheighdata = d3
        .select("#" + "row_data_table" + elementold)
        .node()
        ? d3
        .select("#" + "row_data_table" + elementold)
        .node()
        .getBoundingClientRect().height : 0;
    
    
        if (sizeheightpdf > sizeheightpdfmax) {
            d3.selectAll("#"+elementold).remove();
            
    
            var sisapage = Math.ceil(
                (sizeheightpdf - sizeheightpdfmax) / sizeheighdata
            );
          
            var allpage = 1 ;
    
            if (datatable_page2.length - sisapage !== 0){
    
                 allpage = Math.ceil(
                    datatable_page2.length / (datatable_page2.length - sisapage)
                );
            }
            var page = 1 ;
          
            // console.log( "allpage",allpage )
    
            while (page <= allpage) {
                // console.log("page" , page)
                if (page == allpage) {
                    makeTablepage2("page" + (page +addpage) + idRandom, "nextpage1");
    
                    addImagepage2(
                        "page" + (page +addpage) + idRandom,
                        datatable_page2.slice(
                            0 + (page - 1) * (datatable_page2.length - sisapage),
                            page * (datatable_page2.length - sisapage)
                        ),
                        (page +addpage),
                        allpage +addpage
                    );
    
                    var heightl = d3
                    .select("#" +"page" + (page +addpage) +idRandom)
                    .node()
                    .getBoundingClientRect().height;
                  
                    d3.select("#" + "page" + (page +addpage) +idRandom).style(
                        "margin-bottom",
                        ((sizeheightpdfmax - heightl) + 5 +1 +addmarginbottom) + "px"
                    );

                //    await editpagetext(1, allpage+addpage)
                //     await exportpdfnow(width, file_name);
    
                //     await remove(1, allpage+addpage);

                    addpage = allpage +addpage
                 
    
                
                } else {
                    
                        makeTablepage2("page" + (page +addpage) +idRandom, "nextpage1");
    
                        addImagepage2(
                            "page" + (page +addpage) +idRandom,
                            datatable_page2.slice(
                                0 + (page - 1) * (datatable_page2.length - sisapage),
                                page * (datatable_page2.length - sisapage)
                            ),
                            page +addpage,
                            allpage +addpage
                        );
                        var heightl = d3
                        .select("#" +"page" + (page +addpage) +idRandom)
                        .node()
                        .getBoundingClientRect().height;
                       
                        d3.select("#" + "page" + (page +addpage) +idRandom).style(
                            "margin-bottom",
                            ((sizeheightpdfmax - heightl) + 5 +1+addmarginbottom) + "px"
                        );
                    
                  
                }
    
                page++;
            }
        } else {
       
            d3.selectAll(".row_data_table" + elementold).remove();
            addImagepage2(elementold, datatable_page2, 1+addpage, 1+addpage);
            var heightl = d3
            .select("#"+ elementold)
            .node()
            .getBoundingClientRect().height;


            d3.select("#" + elementold).style(
                "margin-bottom",
                ((sizeheightpdfmax - heightl) + 5 +1+addmarginbottom) + "px"
            );
            // await editpagetext(1,  1+addpage)
            // await exportpdfnow(width, file_name);
            // await remove(1, 1+addpage);
          
            addpage = 1+addpage
       

        }
    }
    


    
    function addImagepage2(elementold, datatableoke, page, allpage) {
        d3.select("#Page" + elementold).text("Page " + page + " of " + allpage);
    
        d3.select("#image_hansell_img" + elementold)
            .attr(
                "src",
                "data:image/png;base64,"
            )
            .attr("width", "119")
            .attr("height", "73")
            .attr("hspace", "21")
            .attr("vspace", "22");
    
        d3.select("#img_image_button" + elementold)
            .attr(
                "src",
                "data:image/png;base64,"
            )
            .attr("width", "442")
            .attr("height", "25")
            .attr("hspace", "185")
            .attr("vspace", "8");
    
            var sum_count_row_data = datatableoke.length ;

            var count_row_data = 1;
            while (count_row_data <= sum_count_row_data) {
                var rowheader1 = d3
                    .select("#tabody_data" + elementold)
                    .append("tr").style("border-right","1px solid #000")
                    .style("border-right","1px solid #000")
                    .attr("id", "row_data_table" + elementold)
                    .attr("class", "row_data_table" + elementold);
                for (let i = 0; i < colgroupdata.length; i++) {
                    if (i == 0) {
                        rowheader1
                            .append("td")
                            .attr("height", "21")
                            .attr("colspan", "4")
                            .style("border-bottom", "1px solid #000000")
                            .style("border-left", "1px solid #000000")
                            .style("text-align", "center")
                            .style("vertical-align", "bottom")
                            .style("font-family", "Arial")
                            .style("font-size", "16px")
                            .style("color", "#000000")
                            .text(function () {
                               
                                    return datatableoke[count_row_data - 1][i];
                                
                             
                            });
                    } else if (i == colgroupdata.length - 1) {
                        // rowheader1
                        //     .append("td")
        
                        //     .style("border-right", "1px solid #000000")
        
                        //     .style("text-align", "left")
                        //     .style("vertical-align", "bottom")
                        //     .style("font-family", "Arial")
                        //     .style("font-size", "16px")
                        //     .style("color", "#000000");
                    } else {
                      
                        if ( i >=2){
                            rowheader1
                            .append("td")
                            .attr("colspan", function () {
            
                                if (i == header_column_page2.length ){
            
                                    return "3"
                                }else {
                                    return "1"
                                }
                               
                              
                            } )
                            .style("border", function () {
                                if (datatableoke[count_row_data - 1]) {
                                    if (
                                        datatableoke[count_row_data - 1][i - 1] !==
                                        undefined
                                    )
                                        return "1px solid #000000";
                                }
                                return "";
                            })
                            .style("text-align", "center")
                            .style("vertical-align", "bottom")
                            .style("font-family", "Arial")
                            .style("font-size", "16px")
                            .style("color", "#000000")
                            .text(function () {
                                if (datatableoke[count_row_data - 1] ) {
                                    return datatableoke[count_row_data - 1][i - 1];
                                }
                                return "";
                            });
        
                        }
                       
                    }
                }
                count_row_data++;
            }
    }

// batas page 2

// function 345






function makeTablepage345(idtable ,configuration,nameoftabledata ,data_from_thingworx) {

    // code 
                    var header_column = [];

                    configuration.forEach((element) => {
                        header_column.push(element.header_name);
                    });

                    datatable = [];

                    data_from_thingworx.forEach((elementi, i) => {
                        var tempdatatable = [];

                        configuration.forEach((elementj, j) => {
                            var tempelemnt = null;

                            if (elementi[elementj.data_shape_column_name]) {
                                tempelemnt = elementi[elementj.data_shape_column_name];
                            }

                            tempdatatable.push(tempelemnt);
                        });

                        datatable.push(tempdatatable);
                    });

                 
    var table = divbody
        .append("table")
        .attr("cellspacing", "0")
        .attr("border", "0")
        .attr("id", idtable)
        // .style("box-sizing", "border-box")
        .style("border-collapse", "collapse")
        .style("table-layout", "fixed")
        .style("width", "fit-content")
        
       


    table
        .selectAll("colgroup")
        .data(colgroupdata)
        .enter()
        .append("colgroup")

        .attr("width", function (d, i) {
            return d.width;
        });

    var tbody = table.append("tbody");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000").attr("id", "header_table" + idtable);
    rowheader1
        .append("td")
        .attr("rowspan", function () {
            if (doc_information.length > 0) return doc_information.length;
            return 1;
        })
        .attr("id", "image_hansell")
        .attr("height", "141")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("vertical-align", "middle")
        .style("font-size", "30px")
        .style("font-weight", "bold")
        .style("font-family", "Arial")
        .style("color", "#000000")

        .text("")
        .append("img")
        .attr("id", "image_hansell_img" + idtable)
        .attr("src", "");

    rowheader1
        .append("td")
        .attr("rowspan", function () {
            if (doc_information.length > 0) return doc_information.length;
            return 1;
        })
        .attr("colspan", colgroupdata.length - 7) //change here
        .style("border", "1px solid #000000")
        .style("vertical-align", "middle")
        .style("font-weight", "bold")
        .style("text-align", "center")
        .style("font-size", "30px")
        .style("font-family", "Arial")
        .style("color", "#000000")
        // .style("text-transform", "uppercase")

        .text(header_name);
    if (doc_information.length == 0) {
        d3.select("#header_table" + idtable)
            .append("td")
            .attr("colspan", "3")
            .style("border", "1px solid #000000")
            .style("vertical-align", "middle")
            .style("text-align", "left")
            .style("font-size", "18px")
            .style("font-family", "Arial")
            .style("color", "#000000")
            .text("");

        d3.select("#header_table" + idtable)
            .append("td")
            .attr("colspan", "3")
            .style("border", "1px solid #000000")
            .style("vertical-align", "middle")
            .style("text-align", "left")
            .style("font-size", "18px")
            .style("font-family", "Arial")
            .style("color", "#000000")
            .text("");
    } else {
        doc_information.forEach((element, i) => {
            if (i == 0) {
                // tr1

                d3.select("#header_table" + idtable)
                    .append("td")
                    .attr("colspan", "3") //change here

                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.name);

                d3.select("#header_table" + idtable)
                    .append("td")
                    .attr("colspan", "3")
                    .attr("id", element.name + idtable)
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.value);
            } else {
                var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
                rowheader1
                    .append("td")
                    .attr("colspan", "3") //change here
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.name);
                rowheader1
                    .append("td")
                    .attr("colspan", "3")
                    .attr("id", element.name + idtable)
                    .style("border", "1px solid #000000")
                    .style("vertical-align", "middle")
                    .style("text-align", "left")
                    .style("font-size", "18px")
                    .style("font-family", "Arial")
                    .style("color", "#000000")

                    .text(element.value);
            }
        });
    }

    var row_header_information = Math.ceil(header_information.length / 5);
    var count_row_header_information = 1;

    //header_information
    while (count_row_header_information <= row_header_information) {
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        var countcolumn = 0;

        header_information
            .slice(
                0 + (count_row_header_information - 1) * 5,
                5 + (count_row_header_information - 1) * 5
            )
            .forEach((element_header_information, i_header_information) => {
        
                countcolumn += 3;
                rowheader1
                    .append("td")
                    .attr("height", "21")
                    .attr("id","header_information")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-left", function () {
                        if (i_header_information == 0) {
                            return "1px solid #000000";
                        }
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")

                    .text(element_header_information.name + " :");

                rowheader1
                    .append("td")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    // .style("background-color", "#FFFF00")

                    .text(element_header_information.value);

                    // if(i_header_information !== 4) {
                        rowheader1
                        .append("td")
                        .style("border-top", function () {
                            if (count_row_header_information == 1)
                                return "1px solid #000000";
                            return "";
                        })
                        .style("border-bottom", function () {
                            if (
                                count_row_header_information ==
                                row_header_information
                            )
                                return "1px solid #000000";
                            return "";
                        })
                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text("");
                    // }
                   
        
            });

        countcolumn++;
        while (countcolumn <= colgroupdata.length) {
            if (countcolumn !== colgroupdata.length) {
                rowheader1
                    .append("td")
                    .attr("id","unoquie")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else {
                rowheader1
                    .append("td")
                    .attr("id","unoquie end")
                    .style("border-top", function () {
                        if (count_row_header_information == 1)
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-right", "1px solid #000000")
                    .style("border-bottom", function () {
                        if (
                            count_row_header_information ==
                            row_header_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            }
            countcolumn++;
        }

        if (count_row_header_information !== row_header_information) {
            var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
            for (let i = 0; i < colgroupdata.length; i++) {
                if (i == 0) {
                    rowheader1
                        .append("td")
                        .attr("height", "21")
                        .style("border-left", "1px solid #000000")
                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text("");
                } else if (i == colgroupdata.length - 1) {
                    rowheader1
                        .append("td")

                        .style("border-right", "1px solid #000000")

                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000");
                } else {
                    rowheader1
                        .append("td")
                        // .style("border-top", "1px solid #000000")

                        .style("text-align", "left")
                        .style("vertical-align", "bottom")
                        .style("font-family", "Arial")
                        .style("font-size", "16px")
                        .style("color", "#000000")
                        .text("");
                }
            }
        }

        count_row_header_information++;
    }

    //dashboar_information

    var row_dashboard_information = Math.ceil(dashboard_information.length / 5);
    var count_row_dashboard_information = 1;

    while (count_row_dashboard_information <= row_dashboard_information) {
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        for (let i = 0; i < colgroupdata.length; i++) {
            if (i == 0) {
                rowheader1
                    .append("td")
                    .attr("height", "21")
                    .style("border-left", "1px solid #000000")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else if (i == colgroupdata.length - 1) {
                rowheader1
                    .append("td")

                    .style("border-right", "1px solid #000000")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            } else {
                rowheader1
                    .append("td")
                    .style("border-top", function () {
                        if (count_row_dashboard_information == 1)
                            return "1px solid #000000";

                        return "";
                    })

                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            }
        }
        var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
        var countcolumn = 0;

        dashboard_information
            .slice(
                0 + (count_row_dashboard_information - 1) * 5,
                5 + (count_row_dashboard_information - 1) * 5
            )
            .forEach((element_header_information, i_header_information) => {
                countcolumn += 3;
                rowheader1
                    .append("td")
                    .attr("height", "21")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("border-left", function () {
                        if (i_header_information == 0) {
                            return "1px solid #000000";
                        }
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text(element_header_information.name + " :");

                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    // .style("background-color", "#FFFF00")
                    .text(element_header_information.value);

                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            });

        countcolumn++;
        while (countcolumn <= colgroupdata.length) {
            if (countcolumn !== colgroupdata.length) {
                rowheader1
                    .append("td")

                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else {
                rowheader1
                    .append("td")

                    .style("border-right", "1px solid #000000")
                    .style("border-bottom", function () {
                        if (
                            count_row_dashboard_information ==
                            row_dashboard_information
                        )
                            return "1px solid #000000";
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            }
            countcolumn++;
        }

        count_row_dashboard_information++;
    }

    var sum_count_row_data =
        datatable.length + 1 > 16 ? datatable.length + 1 : 16;

    var count_row_data = 1;

 

    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    for (let i = 0; i < colgroupdata.length; i++) {
        if (i == 0) {
            rowheader1
                .append("td")
                .attr("height", "21")
                .attr("colspan", "1") 
                .style("border-left", "1px solid #000000")

                .style("text-align", "left")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000")
                .text("");
                

            
        } else if (i == colgroupdata.length - 1) {
            rowheader1
                .append("td")

                .style("border-right", "1px solid #000000")

                .style("text-align", "left")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000");
        } else {

            if ( i == 1){
                rowheader1
                .append("td")
                 .attr("colspan", function () {
                    
                    if (header_column.length == 15){
                        return "14"
                    }else {
                        return header_column.length
                    }
                 } ) 
                .style("border", function () {
                    // if (datatable[count_row_data - 1]) {
                    //     if (datatable[count_row_data - 1][i - 1] !== undefined)
                    //         return "1px solid #000000";
                    // }
                    return "1px solid #000";
                })
                .style("text-align", "center")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000")
                .style("font-weight","bold")
                .style("overflow","hidden")
                .style("white-space","nowrap")
                .style("text-overflow","ellipsis")
                .text(function () {
                    // if (datatable[count_row_data - 1]) {
                    //     return datatable[count_row_data - 1][i - 1];
                    // }
                    return  nameoftabledata ;
                });
            }else if ( i >= header_column.length +1 ){
                rowheader1
                .append("td")
                //  .attr("colspan", header_column.length) 
                .style("border", function () {
                    // if (datatable[count_row_data - 1]) {
                    //     if (datatable[count_row_data - 1][i - 1] !== undefined)
                    //         return "1px solid #000000";
                    // }
                    return "";
                })
                .style("text-align", "left")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000")
                .text(function () {
                    // if (datatable[count_row_data - 1]) {
                    //     return datatable[count_row_data - 1][i - 1];
                    // }
                    return "";
                });
            }
       
        }
    }

    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    for (let i = 0; i < colgroupdata.length; i++) {
        if (i == 0) {
            rowheader1
                .append("td")
                .attr("height", "21")

                .style("border-left", "1px solid #000000")

                .style("text-align", "left")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000")
                .text(function () {
                    "";
                });
        } else if (i == colgroupdata.length - 1) {
            rowheader1
                .append("td")

                .style("border-right", "1px solid #000000")

                .style("text-align", "left")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000");
        } else {
            rowheader1
                .append("td")

                .style("border", function () {
                    if (header_column[i - 1]) {
                        return "1px solid #000000";
                    }
                    return "";
                })
                .style("text-align", "left")
                .style("vertical-align", "bottom")
                .style("font-family", "Arial")
                .style("font-size", "16px")
                .style("color", "#000000")
                .style("overflow","hidden")
                .style("text-overflow","ellipsis")
                .style("white-space","nowrap")

                .text(function () {
                    return header_column[i - 1];
                });
        }
    }

    var tbody = table.append("tbody").attr("id", "tabody_data" + idtable);

    var tbody = table.append("tbody");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    rowheader1
        .append("td")
        .attr("id", "image_buttom")
        .attr("rowspan", "2")
        .attr("colspan", colgroupdata.length)
        .attr("height", "40")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("border-width", "1px 1px 0 1px")
        .style("vertical-align", "bottom")
        .text("")
        .append("img")
        .attr("id", "img_image_button" + idtable)
        .attr("src", "");

    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    var rowheader1 = tbody.append("tr").style("border-right","1px solid #000");
    rowheader1
        .append("td")

        .attr("colspan", colgroupdata.length)
        .attr("height", "30")
        .attr("align", "center")
        .style("border", "1px solid #000000")
        .style("border-width", "0 1px 1px 1px")
        .style("font-style", "italic")
        .style("vertical-align", "center")
        .text("This document is generated from AIMS");


        d3.select("#image_hansell_img" + elementold)
        .attr(
            "src",
            "data:image/png;base64,"
        )
        .attr("width", "119")
        .attr("height", "73")
        .attr("hspace", "21")
        .attr("vspace", "22");

    d3.select("#img_image_button" + elementold)
        .attr(
            "src",
            "data:image/png;base64,"
        )
        .attr("width", "442")
        .attr("height", "25")
        .attr("hspace", "185")
        .attr("vspace", "8");

}




async function converHTMLToCanvaspage345(  elementold, file_name ,elementchild ,nameoftabledata ,data_from_thingworx ,booleanprint) {


     makeTablepage345(elementold , elementchild[0] ,nameoftabledata ,data_from_thingworx );
  

    var sum_count_row_data = datatable.length + 1;

    var count_row_data = 1;
    while (count_row_data <= sum_count_row_data) {
        var rowheader1 = d3
            .select("#tabody_data" + elementold)
            .append("tr").style("border-right","1px solid #000")
            .attr("id", "row_data_table" + elementold)
            .attr("class", "row_data_table" + elementold);
        for (let i = 0; i < colgroupdata.length; i++) {
            if (i == 0) {
                rowheader1
                    .append("td")
                    .attr("height", "21")

                    .style("border-left", "1px solid #000000")

                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else if (i == colgroupdata.length - 1) {
                rowheader1
                    .append("td")

                    .style("border-right", "1px solid #000000")

                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            } else {
                rowheader1
                    .append("td")

                    .style("border", function () {
                        if (datatable[count_row_data - 1]) {
                            if (
                                datatable[count_row_data - 1][i - 1] !==
                                undefined
                            )
                                return "1px solid #000000";
                        }
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text(function () {
                        if (datatable[count_row_data - 1]) {
                            return datatable[count_row_data - 1][i - 1];
                        }
                        return "";
                    });
            }
        }
        count_row_data++;
    }

    var jspdf = new jsPDF("l", "pt", "a4", true);

   
    var width = d3.select("#" + elementold)
        .node()
        .getBoundingClientRect().width;

  
    var height = d3.select("#" + elementold)
        .node()
        .getBoundingClientRect().height;

    


    var sizeheightpdfmax = ((916 ) / 1302.015625) * width;

  

    var sizeheightpdf = height;
    var sizeheighdata = d3
    .select("#" + "row_data_table" + elementold)
    .node()
    .getBoundingClientRect().height;

    // //console.log("sizeheighdata" , sizeheighdata)
    // //console.log( "sizeheightpdf > sizeheightpdfmax",sizeheightpdf , sizeheightpdfmax)

    if (sizeheightpdf > sizeheightpdfmax) {
        d3.selectAll("#"+elementold).remove();
        // d3.selectAll(".row_data_table" + elementold).remove();

        var sisapage = Math.ceil(
            (sizeheightpdf - sizeheightpdfmax) / sizeheighdata
        );
        // //console.log( "banyak data", sisapage )

        // //console.log( "datatable.length", datatable.length )
        var allpage = 1 ;

        if (datatable.length - sisapage !== 0){

             allpage = Math.ceil(
                datatable.length / (datatable.length - sisapage)
            );
        }
        var page = 1;

        // //console.log( "allpage",allpage )
        var heightl = 0
        while (page <= allpage) {
            // //console.log("page" , page)
            if (page == allpage) {
                elementchild.forEach((elementelementchild,ielementchild)=>{

                    makeTablepage345("page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ,elementelementchild ,nameoftabledata ,data_from_thingworx);

                    addImagepage345(
                        "page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ,
                        datatable.slice(
                            0 + (page - 1) * (datatable.length - sisapage),
                            page * (datatable.length - sisapage)
                        ),
                        ((page-1)*elementchild.length + 1+ielementchild) +addpage,
                        allpage*elementchild.length +addpage
                    );
                    var width1 = d3
                    .select("#" +"page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom)
                    .node()
                    .getBoundingClientRect().width;
                

                     sizeheightpdfmax = ((916 ) / 1302.015625) * width1;
                
               

                    heightl = d3
                    .select("#" +"page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom)
                    .node()
                    .getBoundingClientRect().height;

                    
                    if (!booleanprint || ielementchild != elementchild.length -1){
                        d3.select("#" + "page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ).style(
                            "margin-bottom",
                            ((sizeheightpdfmax - heightl) + 5 +1+addmarginbottom) + "px"
                        );

                    }
                   
                
                
                   })
               

              
                   if (booleanprint){
                    await editpagetext(1,allpage*elementchild.length+ addpage)
                    
                    await exportpdfnow(width, file_name);

                    await remove(1,  allpage*elementchild.length+ addpage);
                   }
        


                addpage = allpage*elementchild.length +addpage

                console.log("addpage")
                console.log(addpage)

            
            } else {
      

                  
                    elementchild.forEach((elementelementchild,ielementchild)=>{

                        makeTablepage345("page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ,elementelementchild,nameoftabledata ,data_from_thingworx);

                        addImagepage345(
                            "page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ,
                            datatable.slice(
                                0 + (page - 1) * (datatable.length - sisapage),
                                page * (datatable.length - sisapage)
                            ),
                            ((page-1)*elementchild.length + 1+ielementchild) +addpage,
                            allpage*elementchild.length +addpage
                        );

                    

                        var width1 = d3
                        .select("#" +"page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom)
                        .node()
                        .getBoundingClientRect().width;
                    
    
                         sizeheightpdfmax = ((916 ) / 1302.015625) * width1;
                    
                   

                        heightl = d3
                        .select("#" +"page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom)
                        .node()
                        .getBoundingClientRect().height;

                
                       
                        d3.select("#" + "page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ).style(
                            "margin-bottom",
                            ((sizeheightpdfmax - heightl) + 5 +1+addmarginbottom) + "px"
                        );
                    
                       })
                   
                
                // }
            }

            page++;
        }
    } else {
   
        d3.selectAll("#"+elementold).remove();

        page = 1 
        allpage = 1

        sisapage = 0
        elementchild.forEach((elementelementchild,ielementchild)=>{

            makeTablepage345("page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ,elementelementchild,nameoftabledata ,data_from_thingworx);

            addImagepage345(
                "page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ,
                datatable.slice(
                    0 + (page - 1) * (datatable.length - sisapage),
                    page * (datatable.length - sisapage)
                ),
                ((page-1)*elementchild.length + 1+ielementchild) +addpage,
                allpage*elementchild.length +addpage
            );

            var width1 = d3
            .select("#" +"page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom)
            .node()
            .getBoundingClientRect().width;
        

             sizeheightpdfmax = ((916 ) / 1302.015625) * width1;

          
        
       

            heightl = d3
            .select("#" +"page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom)
            .node()
            .getBoundingClientRect().height;

            if (!booleanprint || ielementchild != elementchild.length -1){

                d3.select("#" + "page" + ((page-1)*elementchild.length + 1+ielementchild +addpage) +idRandom ).style(
                    "margin-bottom",
                    (sizeheightpdfmax - heightl + 5 +1+addmarginbottom) + "px"
                );

            }

           
          
        
           })
     
           if (booleanprint){
               await editpagetext(1,allpage*elementchild.length+ addpage)
                await exportpdfnow(width, file_name);
                await remove(1, allpage*elementchild.length +addpage);
           }
      
        addpage = allpage*elementchild.length +addpage
        console.log("addpage")

        console.log(addpage)
       
    }
}



function addImagepage345(elementold, datatableoke, page, allpage) {
    d3.select("#Page" + elementold).text("Page " + page + " of " + allpage);

    d3.select("#image_hansell_img" + elementold)
        .attr(
            "src",
            "data:image/png;base64,"
        )
        .attr("width", "119")
        .attr("height", "73")
        .attr("hspace", "21")
        .attr("vspace", "22");

    d3.select("#img_image_button" + elementold)
        .attr(
            "src",
            "data:image/png;base64,"
        )
        .attr("width", "442")
        .attr("height", "25")
        .attr("hspace", "185")
        .attr("vspace", "8");

    var sum_count_row_data = datatableoke.length + 1;

    var count_row_data = 1;
    while (count_row_data <= sum_count_row_data) {
        var rowheader1 = d3
            .select("#tabody_data" + elementold)
            .append("tr").style("border-right","1px solid #000")
            .attr("id", "row_data_table" + elementold)
            .attr("class", "row_data_table" + elementold);
        for (let i = 0; i < colgroupdata.length; i++) {
            if (i == 0) {
                rowheader1
                    .append("td")
                    .attr("height", "21")

                    .style("border-left", "1px solid #000000")

                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text("");
            } else if (i == colgroupdata.length - 1) {
                rowheader1
                    .append("td")

                    .style("border-right", "1px solid #000000")

                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000");
            } else {
                rowheader1
                    .append("td")

                    .style("border", function () {
                        if (datatableoke[count_row_data - 1]) {
                            if (
                                datatableoke[count_row_data - 1][i - 1] !==
                                undefined
                            )
                                return "1px solid #000000";
                        }
                        return "";
                    })
                    .style("text-align", "left")
                    .style("vertical-align", "bottom")
                    .style("font-family", "Arial")
                    .style("font-size", "16px")
                    .style("color", "#000000")
                    .text(function () {
                        if (datatableoke[count_row_data - 1]) {
                            return datatableoke[count_row_data - 1][i - 1];
                        }
                        return "";
                    });
            }
        }
        count_row_data++;
    }
}
      
    };
};
